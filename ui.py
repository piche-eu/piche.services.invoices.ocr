...
def parse_with_tesseract(filepath):
    parsed_file = pytesseract.image_to_data(Image.open(filepath), output_type='data.frame')
    return parsed_file
...
def parse_with_paddle(filepath):
    ...
    im_path = os.path.dirname(filepath)+"/png/output" + os.path.basename(filepath)+".png"
    return text, im_path
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        dropdown = gr.Dropdown(
            ["llava", "docling", "tesseract", "paddle", "granite", "gemma"], label="OCR", interactive=True, info="OCR Engine"
        )
    with gr.Row():
